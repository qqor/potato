...
def process(cmd, fn, funcname):
    global gfname
    bp = os.getcwd()
    with cd("~/pin-3.0-76991-gcc-linux/source/tools/IMFSIM"):
        lines = []
        warning(cmd)
...
def new_cmd(fn, baddr, eaddr, ce, cr):
    runcmd = "../../../pin -ifeellucky -t ./obj-intel64/process.so -start {0} -end {1} -codeStart {2} -codeRange {3} -fuzzingCount {4} -fuzzingType step -startValue 0x1 -- {5} > temp.out"
